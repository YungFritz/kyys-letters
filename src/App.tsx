...
/* ========= Données (seed) — mêmes structures que ton home original ========= */
const SEED: Series[] = [
...
const load = (): Series[] => {
  try { const raw = localStorage.getItem(LS_KEY); return raw ? JSON.parse(raw) as Series[] : SEED; }
  catch { return SEED; }
};
...
/* ========= Mini-router en hash ========= */
type Route = { name: "home" } | { name: "admin" } | { name: "admin-new" } | { name: "admin-edit"; slug: string };
...
/* ========= Header (reprend ton header original, j’ajoute juste un lien Admin) ========= */
...
/* ========= Carte (identique à ton placeholder sans image) ========= */
...
/* ========= HOME — strictement ton layout d’origine ========= */
function Home({ library }: { library: Series[] }) {
  const [query, setQuery] = useState("");
  const popular = useMemo(()=> library.slice().sort((a,b)=> (b.views||0)-(a.views||0)).slice(0,8), [library]);
        ...
        <div style={{ display:"grid", gridTemplateColumns:"3fr 1fr", gap:18, marginTop:20 }}>
          <div>
            ...
            <div className="latest-grid">
              {library.flatMap(series => series.chapters.map(chapter => ({series, chapter})))
...
/* ========= Admin (structure simple, pas de CSS intrusif) ========= */
...
const btn = (b:string,c:string)=>({border:`1px solid ${b}`,background:c,color:"#e5e7eb",padding:"8px 12px",borderRadius:10});
...
function AdminDashboard({ library, onDelete }: { library: Series[]; onDelete:(slug:string)=>void }) {
  ...
  return (
    <AdminLayout title="Séries" right={<button onClick={()=>nav("/admin/manga/new")} style={btn("#3a2d12","#1d1405")}>+ Nouvelle série</button>}>
      <input value={q} onChange={e=>setQ(e.target.value)} placeholder="Rechercher" style={{ width:"100%", border:"1px solid #27272a", background:"#0f1012", color:"#e5e7eb", padding:"10px 12px", borderRadius:12, marginBottom:12 }}/>
      <div style={{ display:"grid", gap:12 }}>
        {list.map(s=>(
...
function AdminAdd({ onCreate }:{ onCreate:(s:Series)=>void }) {
  const [title,setTitle]=useState(""); const [tags,setTags]=useState("FR"); const [slug,setSlug]=useState("");
  const makeSlug=(t:string)=>t.toLowerCase().normalize("NFD").replace(/\p{Diacritic}/gu,"").replace(/[^a-z0-9]+/g,"-").replace(/(^-|-$)/g,"");
  const post=()=>{ if(!title.trim())return; const sg=slug||makeSlug(title); onCreate({id:`s-${crypto.randomUUID()}`,title:title.trim(),slug:sg,tags:tags.split(",").map(x=>x.trim()).filter(Boolean),chapters:[],views:0}); nav(`/admin/manga/${sg}/edit`); };
  return (
    <AdminLayout title="Ajouter une série" right={<button onClick={post} style={btn("#2b4c18","#0c1409")}>Post</button>}>
      <div style={{ display:"grid", gap:12 }}>
        <input value={title} onChange={e=>setTitle(e.target.value)} placeholder="Titre…" style={{ width:"100%", border:"1px solid #27272a", background:"#141519", color:"#e5e7eb", padding:"10px 12px", borderRadius:10 }}/>
        <input value={slug} onChange={e=>setSlug(e.target.value)} placeholder="Slug (optionnel)" style={{ width:"100%", border:"1px solid #27272a", background:"#141519", color:"#e5e7eb", padding:"10px 12px", borderRadius:10 }}/>
        <input value={tags} onChange={e=>setTags(e.target.value)} placeholder="Tags (séparés par virgules)" style={{ width:"100%", border:"1px solid #27272a", background:"#141519", color:"#e5e7eb", padding:"10px 12px", borderRadius:10 }}/>
      </div>
    </AdminLayout>
  );
}

/* Modifier série + ajouter chapitre (structure) */
function AdminEdit({ library, slug, onSave, onAddChapter, onDelete }:{
  library: Series[]; slug:string;
  onSave:(s:Series)=>void; onAddChapter:(slug:string, ch:Chapter)=>void; onDelete:(slug:string)=>void;
}) {
  ...
  const [num,setNum]=useState(1); const [name,setName]=useState(""); const [date,setDate]=useState<string>(new Date().toISOString().slice(0,10)); const [lang,setLang]=useState("FR");

  useEffect(()=>{ const f=library.find(x=>x.slug===slug); if(f) setS(f);},[slug,library]);
  const save=()=>onSave(s);
  const add=()=>{ const ch:Chapter={id:`c-${crypto.randomUUID()}`,name: name||`Chapitre ${num}`, number:num, lang, releaseDate:date, pages:[]}; onAddChapter(s.slug,ch); setName(""); };

  return (
    <AdminLayout title={`Modifier : ${s.title}`} right={<button onClick={save} style={btn("#2b4c18","#0c1409")}>Save</button>}>
      <div style={{ display:"grid", gap:12 }}>
        <input value={s.title} onChange={e=>setS({...s,title:e.target.value})} style={{ width:"100%", border:"1px solid #27272a", background:"#141519", color:"#e5e7eb", padding:"10px 12px", borderRadius:10 }}/>
        <input value={s.tags.join(", ")} onChange={e=>setS({...s,tags:e.target.value.split(",").map(x=>x.trim()).filter(Boolean)})} style={{ width:"100%", border:"1px solid #27272a", background:"#141519", color:"#e5e7eb", padding:"10px 12px", borderRadius:10 }}/>
        ...
        <div style={{ borderTop:"1px solid #1d1d22", paddingTop:12, marginTop:6 }}>
          ...
          <div style={{ display:"grid", gap:12, gridTemplateColumns:"1fr 1fr 1fr 1fr" }}>
            ...
            <select value={lang} onChange={e=>setLang(e.target.value)} style={{ ...inp }}><option>FR</option><option>EN</option><option>JP</option></select>
          ...
          <div style={{ display:"grid", gap:10 }}>
            {s.chapters.slice().sort((a,b)=>b.number-a.number).map(c=>(
              <div key={c.id} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", border:"1px solid #27272a", background:"#0f0f12", borderRadius:12, padding:"10px 12px" }}>
                <div><strong>Chapitre {c.number}</strong> — {c.name} <span style={{ color:"#9aa0a6" }}>({c.lang} • {c.releaseDate})</span></div>
...
export default function App(){
  ...
  useEffect(()=>{ const h=()=>setRoute(parseRoute(window.location.hash)); window.addEventListener("hashchange",h); return ()=>window.removeEventListener("hashchange",h);},[]);
  ...
  const deleteSeries=(slug:string)=> { setLibrary(prev=>prev.filter(s=>s.slug!==slug)); nav("/admin"); };
  const addChapter=(slug:string,ch:Chapter)=> setLibrary(prev=>prev.map(s=> s.slug===slug ? {...s,chapters:[...s.chapters,ch]} : s));
  ...
  return (
    <>
      ...
      {route.name==="admin-edit" && <AdminEdit library={library} slug={route.slug} onSave={updateSeries} onAddChapter={addChapter} onDelete={deleteSeries} />}
    </>
  );
}
